...
class Crawler:
    def __init__(self, query: str, pages: int, location: str):
        self.browser = None
        self.page = None
    ...
    def scrape_indeed(self):
        ...
        for _ in range(self.pages):
            ...
            for job in tqdm(job_elements):
                ...
                company_name = (
                    self.page.query_selector('div[data-company-name="true"]')
                    .inner_text()
                    .strip()
                )
                ...
                job_description = (
                    self.page.query_selector("div.jobsearch-JobComponent-description")
                    .inner_text()
                    .strip()
                )
    ...
    def scrape_foundit(self):
        ...
        self.page.fill(
            "input#heroSectionDesktop-locationAutoComplete--input", self.location
        )
...
async def scrape_indeed(commons: Crawler = Depends(Crawler)):
    commons.page.goto("https://in.indeed.com/")
    commons.page.wait_for_selector("input#text-input-what")
    commons.page.fill("input#text-input-what", commons.query)
    commons.page.fill("input#text-input-where", commons.location)
    commons.page.click("button.yosegi-InlineWhatWhere-primaryButton")

    commons.page.click("span#dateLabel")
    commons.page.wait_for_timeout(2000)

    post_counter = 0
    job_data = []

    response = {}
    for _ in range(commons.pages):
        # self.page.wait_for_selector('.jobTitle.css-198pbd.eu4oa1w0', timeout=120000)
        job_elements = commons.page.query_selector_all(".jobTitle.css-198pbd.eu4oa1w0")
        for job in tqdm(job_elements):
            job.click(timeout=120000)
            commons.page.wait_for_timeout(5000)

            job_title = commons.page.query_selector("h2.jobsearch-JobInfoHeader-title")
            if job_title:
                job_title = job_title.inner_text().strip()
            company_name = (
                commons.page.query_selector('div[data-company-name="true"]')
                .inner_text()
                .strip()
            )
            # company_rating = self.page.query_selector('span.css-ppxtlp.e1wnkr790').inner_text().strip()
            location = (
                commons.page.query_selector(
                    'div[data-testid="inlineHeader-companyLocation"]'
                )
                .inner_text()
                .strip()
            ...
            job_description = (
                commons.page.query_selector("div.jobsearch-JobComponent-description")
                .inner_text()
                .strip()
            )
        ...
        # Create a DataFrame
        next_button = commons.page.query_selector(
            'a[data-testid="pagination-page-next"]'
        )
        if next_button or post_counter < 1:
            next_button.click()
            commons.page.wait_for_timeout(5000)  # Wait for the next page to load
            post_counter += 1
        else:
            break
    ...
    # df = pd.DataFrame(job_data)
    # os.makedirs("./data", exist_ok=True)
    # df.to_csv(f"./data/Jobs_indeed_{commons.today}.csv")
